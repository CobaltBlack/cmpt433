...
JoystickDirection Joystick_getInput()
{
	...
	concatPath(JOYSTICK_UP_PATH, VALUE_FILE, directionPath);
	if (readFromFile(directionPath, pinValue)) {
		if (pinValue[0] == PIN_VALUE_0) {
			return JOYSTICK_UP;
		}
	}

	concatPath(JOYSTICK_DOWN_PATH, VALUE_FILE, directionPath);
	if (readFromFile(directionPath, pinValue)) {
		if (pinValue[0] == PIN_VALUE_0) {
	...
	concatPath(JOYSTICK_LEFT_PATH, VALUE_FILE, directionPath);
	if (readFromFile(directionPath, pinValue)) {
		if (pinValue[0] == PIN_VALUE_0) {
	...
	concatPath(JOYSTICK_RIGHT_PATH, VALUE_FILE, directionPath);
	if (readFromFile(directionPath, pinValue)) {
		if (pinValue[0] == PIN_VALUE_0) {
	...
	concatPath(JOYSTICK_PUSH_PATH, VALUE_FILE, directionPath);
	if (readFromFile(directionPath, pinValue)) {
		if (pinValue[0] == PIN_VALUE_0) {
